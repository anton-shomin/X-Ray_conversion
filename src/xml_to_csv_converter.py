...
def xml_to_csv(dir_path, repository_path):
  ...
  Args:
      ...
      repository_path (str): The path to the test repository.
  ...
  try:
    with open(csv_file_path, mode='w', newline='') as file:
      ...
      writer.writerow(["Issue Id", "Test Summary", "Description", "Action", "Data", "Result",
                       "Labels", "Labels", "Labels", "Test Repository Path"])
      for i, filename in enumerate(os.listdir(dir_path), start=1):
        if filename.endswith(".xml"):
          ...
          for j, step in enumerate(steps):
            ...
            # Include repository path only for the first step of each test case
            repo_path = repository_path if j == 0 else ""

            writer.writerow([i, summary if j == 0 else "", description if j == 0 else "",
                             action, data, result] + labels + [repo_path])
